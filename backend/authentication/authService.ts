...
// interface is used to describe the shape of the object - its properties and types
// used by compiler to catch mistakes
interface TwoFASecret {
  qr: string;
  secret: string;
}

export async function generate2FASecret(username: string, userID: number): Promise<TwoFASecret> { // tells the compiler what ths function returns
