...
export default function Input({id, label, type = "text", value, tooltip, autofocus=false, onChange, onBlur,}: InputProps) {
	...
	return (
		<div className="flex flex-col m-2 px-4 text-black dark:text-white">
          ...
          {tooltip}
			...
			<input className="p-1 border border-white rounded-sm" type={type} id={id} value={value} autoFocus={autofocus} onChange={onChange} onBlur={onBlur}/>
		</div>
	)
}